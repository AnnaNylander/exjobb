...
class ClusterNet(nn.Module):
    ''' Performs classification on the type of trajectory and regression on
    adjustments to the trajectory principal components. '''
    def __init__(self, n_past_lidars, n_past_steps, n_future_steps, n_pc, n_clusters):
        ...
        self.linear_d3 = nn.Linear(800, 256)

        # This is for calculating the class probability distribution
        self.class_linear = nn.Linear(256, n_clusters)

        # This is for calculating actual
        self.delta_linear = nn.Linear(256, n_clusters*n_pc)
    ...
    def forward(self, l, v):
        ...
        # PART 1: classes
        class_logits = self.class_linear(y)

        # PART 2: principal component deltas
        pc_deltas = self.delta_linear(y)

        return pc_deltas, class_logits
