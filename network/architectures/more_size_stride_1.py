...
class CNNBiasAll(nn.Module):
    ...
    def __init__(self, n_past_lidars, n_past_steps, n_future_steps):
        ...
        # encoder
        self.conv_e0 = nn.Conv2d(n_past_lidars+1, 8, 5, padding=2)
        self.bias_e0 = nn.Linear(self.vs, 8 ) # 330 x 8

        self.conv_e1 = nn.Conv2d(8,8, 5, padding=2)
        self.bias_e1 = nn.Linear(self.vs, 8 ) # 330 x 8
        self.maxpool_e1 = nn.MaxPool2d(2, stride=2)

        self.conv_e2 = nn.Conv2d(8,16, 5, padding=2)
        self.bias_e2 = nn.Linear(self.vs, 16 ) # 330 x 8

        self.conv_e3 = nn.Conv2d(16,16, 5, padding=2)
        ...
        self.conv_c0 = nn.Conv2d(16,96,5, padding=2, dilation=1)
        self.bias_c0 = nn.Linear(self.vs,96 ) # 330 x 8

        self.conv_c1 = nn.Conv2d(96,96,5, padding=2, dilation=1)
        self.bias_c1 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c2 = nn.Conv2d(96,96,5, padding=(4,2), dilation=(2,1))
        self.bias_c2 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c3 = nn.Conv2d(96,96,5, padding=(8,4), dilation=(4,2))
        self.bias_c3 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c4 = nn.Conv2d(96,96,5, padding=(16,8), dilation=(8,4))
        self.bias_c4 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c5 = nn.Conv2d(96,96,5, padding=(24,16), dilation=(12,8))
        self.bias_c5 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c6 = nn.Conv2d(96,96,5, padding=(32,24), dilation=(16,12))
        self.bias_c6 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c7 = nn.Conv2d(96,96,5, padding=(40,32), dilation=(20,16))
        self.bias_c7 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c8 = nn.Conv2d(96,96,5, padding=(48,40), dilation=(24,20))
        self.bias_c8 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c9 = nn.Conv2d(96,96,5, padding=(56,48), dilation=(28,24))
        self.bias_c9 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c10 = nn.Conv2d(96,96,5, padding=(64,56), dilation=(32,28))
        self.bias_c10 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c11 = nn.Conv2d(96,96,5, padding=(2,64), dilation=(1,32))
        self.bias_c11 = nn.Linear(self.vs, 96 ) # 330 x 8

        self.conv_c12 = nn.Conv2d(96,16,5, padding=2, dilation=1)
        self.bias_c12 = nn.Linear(self.vs, 16 ) # 330 x 8

        # decoder convolutions # 150 x 150 x 16
        self.conv_d0 = nn.Conv2d(16,16,5, padding=2)
        self.bias_d0 = nn.Linear(self.vs, 16 ) # 330 x 8
        self.maxpool_d0 = nn.MaxPool2d(3, stride=3)            # 50 x 50 x 16

        self.conv_d1 = nn.Conv2d(16,16,5, padding=2)
