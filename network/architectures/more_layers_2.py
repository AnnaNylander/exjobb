...
class CNNBiasAll(nn.Module):
    ...
    def __init__(self, n_past_lidars, n_past_steps, n_future_steps):
        ...
        self.conv_e8 = nn.Conv2d(32,36,3, padding=1)
        self.bias_e8 = nn.Linear(self.vs, 36 ) # 330 x 8

        # Lägg in fler lager här och i slutet
        self.conv_e9 = nn.Conv2d(36,40,3, padding=1)
        self.bias_e9 = nn.Linear(self.vs, 40 ) # 330 x 8

        self.conv_e10 = nn.Conv2d(40,44,3, padding=1)
        self.bias_e10 = nn.Linear(self.vs, 44 ) # 330 x 8

        self.conv_e11 = nn.Conv2d(44,48,3, padding=1)
        self.bias_e11 = nn.Linear(self.vs, 48 ) # 330 x 8

        self.conv_e12 = nn.Conv2d(48,52,3, padding=1)
        self.bias_e12 = nn.Linear(self.vs, 52 ) # 330 x 8

        self.conv_e13 = nn.Conv2d(52,56,3, padding=1)
        self.bias_e13 = nn.Linear(self.vs, 56 ) # 330 x 8

        self.conv_e14 = nn.Conv2d(56,60,3, padding=1)
        self.bias_e14 = nn.Linear(self.vs, 60 ) # 330 x 8

        self.conv_e15 = nn.Conv2d(60,64,3, padding=1)
        self.bias_e15 = nn.Linear(self.vs, 64 ) # 330 x 8
    ...
    def forward(self, l, v):
        ...
        b = F.elu(self.bias_e11(v))                             # [2, 8]
        b = expand_biases(b,150,150)                            # [2, 8, 600, 600]
        l = F.elu(torch.add(self.conv_e11(l), 1, b))            # [2, 8, 600, 600]

        b = F.elu(self.bias_e12(v))                             # [2, 8]
        b = expand_biases(b,150,150)                            # [2, 8, 600, 600]
        l = F.elu(torch.add(self.conv_e12(l), 1, b))            # [2, 8, 600, 600]

        b = F.elu(self.bias_e13(v))                             # [2, 8]
        b = expand_biases(b,150,150)                            # [2, 8, 600, 600]
        l = F.elu(torch.add(self.conv_e13(l), 1, b))            # [2, 8, 600, 600]

        b = F.elu(self.bias_e14(v))                             # [2, 8]
        b = expand_biases(b,150,150)                            # [2, 8, 600, 600]
        l = F.elu(torch.add(self.conv_e14(l), 1, b))            # [2, 8, 600, 600]

        b = F.elu(self.bias_e15(v))                             # [2, 8]
        b = expand_biases(b,150,150)                            # [2, 8, 600, 600]
        l = F.elu(torch.add(self.conv_e15(l), 1, b))            # [2, 8, 600, 600]
